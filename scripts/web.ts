...
const child=spawn(process.execPath,[resolve('apps/web/node_modules/next/dist/bin/next'),process.argv.includes('--dev')?'dev':'start','--hostname','127.0.0.1','--port',String(webPort)],{cwd:resolve('apps/web'),env:process.env,stdio:'inherit',windowsHide:true});
