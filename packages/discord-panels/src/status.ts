...
export function experimentEvidenceLabel(status:string,locale:UiLocale='en'){const key=evidenceKeys[status];return key?t(locale,key):t(locale,'evidence.insufficient');}
...
export function diagnosisLabel(type:string,locale:UiLocale='en'){const key=diagnosisKeys[type];return key?t(locale,key):t(locale,'common.needsAttention');}
