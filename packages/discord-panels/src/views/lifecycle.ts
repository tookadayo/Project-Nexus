...
export async function lifecyclePanel(issue:Issue,metrics:Metrics,locale:UiLocale='en'):Promise<Panel>{return nexusPanel({title:t(locale,'area.journey'),subtitle:t(locale,'area.journeySubtitle'),children:[divider(),
 ...
 ],rows:[await actionRow(issue,[{label:discordLabel(locale,'common.diagnose'),action:'improve',style:ButtonStyle.Primary},{label:discordLabel(locale,'common.overview'),action:'overview'},{label:discordLabel(locale,'common.refresh'),action:'lifecycle'}])]});}
