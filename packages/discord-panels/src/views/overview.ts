...
export async function overviewPanel(issue:Issue,metrics:Metrics,locale:UiLocale='en',context:OverviewContext={}):Promise<Panel>{
 ...
 ],rows:[await actionRow(issue,[{label:discordLabel(locale,'overview.createExperiment'),action:'improve',style:ButtonStyle.Primary},{label:discordLabel(locale,'common.lifecycle'),action:'lifecycle'},{label:discordLabel(locale,'common.refresh'),action:'overview'}])]});
}
