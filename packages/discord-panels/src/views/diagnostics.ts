...
export async function diagnosticsPanel(issue:Issue,findings:DiagnosisView[],locale:UiLocale='en'):Promise<Panel>{
 ...
 return nexusPanel({title:t(locale,'area.opportunities'),subtitle:t(locale,'area.opportunitiesSubtitle'),accent:findings.some(f=>f.severity==='critical')?'critical':findings.length?'warning':'healthy',children:[divider(),...children,divider(),footer(t(locale,'diagnostics.footer'))],rows:[await actionRow(issue,[{label:discordLabel(locale,'root.actions'),action:'interventions',style:ButtonStyle.Primary},{label:discordLabel(locale,'root.viewJourney'),action:'lifecycle'},{label:discordLabel(locale,'common.refresh'),action:'diagnose'}])]});
}
