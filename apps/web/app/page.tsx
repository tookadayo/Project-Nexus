...
export default async function Page(){
 ...
 const data:ProductData={home:null,journey:null,community:null,opportunities:null,actions:null,results:null,weeklyStatus:null,options:{channels:[],roles:[],events:[],available:false},admin:null,guilds:context.guilds,selectedGuildId:context.guildId};
 {
  ...
  const [home,journey,community,opportunities,actions,results,weeklyStatus,options,admin]=await Promise.all([read<HomePresentation>(base+'/home'),read<JourneyPresentation>(base+'/journey?range=30'),read<Awaited<ReturnType<CommunityService['overview']>>>(base+'/community?range=30'),read<OpportunitiesPresentation>(base+'/opportunities'),read<ActionsPresentation>(base+'/actions'),read<ResultsPresentation>(base+'/results'),read<ProductData['weeklyStatus']>(base+'/weekly-summary/status'),read<DiscordOptions>(base+'/options'),read<ProductData['admin']>(`${context.base}/v2/organizations/${context.organizationId}/guilds/${context.guildId}/dashboard`)]);
  Object.assign(data,{home,journey,community,opportunities,actions,results,weeklyStatus,options:options??data.options,admin});
 }
